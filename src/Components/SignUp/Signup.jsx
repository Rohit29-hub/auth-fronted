...
const Signup = (props) => {
  ...
  const checkUserSignIn = useNavigate();
  ...
  function sendForm(e) {
    ...
    try {
      if (user.email === "") {
        alert("Email is not be blank");
      } else if (!user.email.endsWith("@gmail.com")) {
        alert("Please enter right email");
      } else if (user.password !== user.rpassword) {
        alert("Password is not match");
      } else if (user.password === "" && user.rpassword === "") {
        alert("Password is not be blank");
      }else if(user.firstname === ''){
          alert('Please Provide your name')
      }else if(user.phone.length !== 10){
        alert('Your phone number is not valid')
      }else {
        axios.post(`http://localhost:4000/sendform`, user).then(() => {
          checkUserSignIn('/about',{state : user})
          props.chnageStatus(true);
          props.userData(user);
        });
      }
    } catch (err) {
      console.log("Error to send data : ", err);
    }
  }

  return (
    <>
      <form className="form-container" style={{ width: "80%", margin: "50px auto" }}>
